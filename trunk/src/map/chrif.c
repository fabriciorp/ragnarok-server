...
void chrif_authok(int fd)
{
	...
	{
		ShowError("chrif_authok: Tamanho de dados incompativeis! %d != %d\n", RFIFOW(fd,2) - 24, sizeof(struct mmo_charstatus));
		return;
	}
...
{
	if (battle_config.etc_log)
		ShowInfo("chrif_changeemail: conta: %d, email atual: '%s', novo email: '%s'.\n", id, actual_email, new_email);
...
int chrif_changesex(struct map_session_data *sd)
{
	...
	clif_displaymessage(sd->fd, "Necess·rio desconectar para realizar a mudanÁa de sexo...");
...
static void chrif_char_ask_name_answer(int acc, const char* player_name, uint16 type, uint16 answer)
{
	...
	switch( type ) {
	case 1 : action = "bloquear"; break;
	case 2 : action = "banir"; break;
	case 3 : action = "desbloquear"; break;
	case 4 : action = "desbanir"; break;
	case 5 : action = "mudar de sexo"; break;
	default: action = "???"; break;
	}
	
	switch( answer ) {
	case 0 : sprintf(output, "O Servidor de Login foi perguntado para %s o personagem '%.*s'.", action, NAME_LENGTH, player_name); break;
	case 1 : sprintf(output, "O personagem '%.*s' n„o existe.", NAME_LENGTH, player_name); break;
	case 2 : sprintf(output, "Seu nÌvel de GM n„o lhe autoriza a %s o personagem '%.*s'.", action, NAME_LENGTH, player_name); break;
	case 3 : sprintf(output, "O Servidor de Login est· desligado. ImpossÌvel %s o personagem '%.*s'.", action, NAME_LENGTH, player_name); break;
...
int chrif_changedsex(int fd)
{
	...
	if (sd) { //Normally there should not be a char logged on right now!
		...
		clif_displaymessage(sd->fd, "Seu sexo foi alterado (necess·rio desconectar do servidor)...");
...
int chrif_parse(int fd)
{
	...
	if (fd != char_fd)
	{
		ShowDebug("chrif_parse: Desconectando sessao invalida #%d (nao e o servidor de personagens)\n", fd);
		do_close(fd);
		return 0;
	}
	...
	while (RFIFOREST(fd) >= 2)
	{
		...
		if (cmd < 0x2af8 || cmd >= 0x2af8 + ARRAYLENGTH(packet_len_table) || packet_len_table[cmd-0x2af8] == 0)
		{
			...
			ShowWarning("chrif_parse: sessao #%d, intif_parse falhou (comando 0x%.4x nao reconhecido).\n", fd, cmd);
			set_eof(fd);
			return 0;
		}
