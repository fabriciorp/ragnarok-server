...
int battle_delay_damage (unsigned int tick, int amotion, struct block_list *src, struct block_list *target, int attack_type, int skill_id, int skill_lv, int damage, enum damage_lv dmg_lv, int ddelay)
{
	...
	if( sc && sc->data[SC_DEVOTION] && damage > 0 
		&& attack_type > 0 && skill_id != PA_PRESSURE )  // Correção do alvo de reflexão do dano (mágico e físico) e do gloria domini ~ [ItachiSan] 
		damage = 0;
...
int battle_attr_fix(struct block_list *src, struct block_list *target, int damage,int atk_elem,int def_type, int def_lv)
{
	...
	if (def_type < 0 || def_type > ELE_MAX ||
		def_lv < 1 || def_lv > 4) {
		ShowError("battle_attr_fix: tipo de atributo desconhecido: atq=%d tipo_de_def=%d nivel_de_defesa=%d\n",atk_elem,def_type,def_lv);
		return damage;
	}
			...
			default:
			{
				...
				if (nk&NK_SPLASHSPLIT){ // Divide ATK among targets
					if(wflag>0)
						...
						ShowError("Nenhum inimigo foi alvo da habilidade %d:%s , evitando divisao por 0!\n", skill_num, skill_get_name(skill_num));
				}
...
#define MATK_ADD( a ) { ad.damage+= a; }
		...
		{	//Adds atk2 to the damage, should be influenced by number of hits and skill-ratio, but not mdef reductions. [Skotlex]
			...
			if(mflag>0)
				...
				ShowError("Nenhum alcance por %d:%s, evitando divisao por 0!\n", skill_num, skill_get_name(skill_num));
		}
...
//	{ "mob_skill_use",                      &battle_config.mob_skill_use,                   1,      0,      1,              }, //Deprecated
	...
	{ "max_heal",                           &battle_config.max_heal,                        99999,   0,      INT_MAX,        },
	...
	{ "max_lv",                             &battle_config.max_lv,                          1000,     0,      127,            },
...
int battle_set_value(const char* w1, const char* w2)
{
	...
	{
		ShowWarning("Valor para configuracao '%s': %s e invalido (min:%i max:%i)! Padronizando para %i...\n", w1, w2, battle_data[i].min, battle_data[i].max, battle_data[i].defval);
...
void battle_adjust_conf()
{
	...
	if (battle_config.cell_stack_limit != 1)
		ShowWarning("Configuracao de Batalha 'cell_stack_limit' nao produz nenhum efeito se esse servidor for compilado sem o suporte a Cell Stack Limit.\n");
#endif
}
	...
	else
	{
		while(fgets(line, sizeof(line), fp))
		{
			...
			if (sscanf(line, "%1023[^:]:%1023s", w1, w2) != 2)
				continue;
			if (strcmpi(w1, "import") == 0) // Não era para traduzir - [ItachiSan]
				battle_config_read(w2);
			else
			if (battle_set_value(w1, w2) == 0)
				ShowWarning("Configuracao '%s' desconhecida no arquivo %s\n", w1, cfgName);
		}
