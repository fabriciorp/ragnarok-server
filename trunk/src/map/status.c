...
int status_check_skilluse(struct block_list *src, struct block_list *target, int skill_num, int flag)
{
	...
	// BA_DISSONANCE tamb駑 remove o hide do alvo
...
static unsigned short status_calc_batk(struct block_list *bl, struct status_change *sc, int batk)
{
	...
	if(sc->data[SC_INCBASEATK]) // usado no doce hiper acucarado, que eh cumulativo com SC_ATKPOTION, SC_BATKFOOD, etc. ~ [ItachiSan]
...
static short status_calc_aspd_rate(struct block_list *bl, struct status_change *sc, int aspd_rate)
{
	...
	if(sc->data[SC_INCASPDRATE]) // [ItachiSan] ~ usado no doce hiper acucarado, que eh cumulativo com SC_ASPDPOTION0~3, etc. ~ [ItachiSan]
