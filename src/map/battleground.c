...
{ // Single Player leaves team
	...
	if( flag )
		sprintf(output, "Servidor : %s abandonou o jogo...", sd->status.name);
	else
		sprintf(output, "Servidor : %s está abandonando o campo de batalha...", sd->status.name);
	clif_bg_message(bg, 0, "Servidor", output, strlen(output) + 1);
